...
def batch_sample(data, all_config,endpoint_list):
    col = all_config['the_endpoint']
    batch_data = data.groupby(col).sample(n=all_config['batch_size'],replace=True)
...
def batch_sample_multi(train_df,all_config):
    ...
    batch = train_df.groupby(['endpoint', 'y']).sample(n=all_config['batch_size'],replace=True)
...
def eval_fn(train_df,all_config,model,endpoint_list):
    ...
    for t in range(all_config['eval_step']):
        ...
        elif all_config['exp_setting'] in ['multi_task_baseline' ,'multi_task_ood','multi_task_share_till_logit']:
            head_config=all_config['head_config']
            batch = train_df.groupby(['endpoint', 'y']).sample(n=all_config['batch_size'],replace=True)
            batch_smiles = batch['similes'].values
            batch_y = batch['y'].values
...
#
# def batch_sample_imbalance(data, all_config,endpoint_list):
#     col = all_config['the_endpoint']
#     batch_data = data.groupby(col).sample(n=all_config['batch_size'],replace=True)
#
#     batch_smiles = batch_data['smiles'].values.tolist()
...
def eval_fn_imbalance(train_df,all_config,model,endpoint_list):
    eval_logit = []
    eval_target = []
    for t in range(all_config['eval_step']):
        if all_config['exp_setting']=='single_task':
            ...
            head_config=all_config['head_config']
            batch = train_df.groupby(['endpoint']).sample(n=all_config['batch_size']*2,replace=True)
            batch_smiles = batch['similes'].values
            batch_y = batch['y'].values
            batch_target = torch.tensor(batch_y.reshape((head_config['channel_size'],
                                                         head_config['batch_size'] * head_config[
